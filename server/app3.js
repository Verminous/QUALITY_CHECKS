const express = require("express"), ExcelJS = require('exceljs'), excel4node = require('excel4node'), cors = require('cors'), fs = require('fs'), multer = require("multer"), xlsx = require("xlsx"), bodyParser = require("body-parser"), path = require("path"), app = express(), upload = multer({ dest: "uploads/" });
...
const {
  ...
  mapIncidentsByAgent = (data) => data.reduce((acc, incident) => { const agent = incident["Taken By"]; acc[agent] = acc[agent] || []; const existingIncidentIndex = acc[agent].findIndex(i => i["Task Number"] === incident["Task Number"]); if (existingIncidentIndex !== -1) { acc[agent][existingIncidentIndex] = { ...acc[agent][existingIncidentIndex], ...incident }; } else { acc[agent].push(incident); } return acc; }, {}),
  ...
  selectUniqueIncidentForAgent = (filteredIncidents = [], alreadySelected, originalIncidents = []) => { let uniqueIncidents = filteredIncidents.filter((incident) => !alreadySelected.has(incident['Task Number'])); if (!uniqueIncidents.length) { uniqueIncidents = originalIncidents.filter((incident) => !alreadySelected.has(incident['Task Number'])); } let selectedIncident = uniqueIncidents.length ? uniqueIncidents[Math.floor(Math.random() * uniqueIncidents.length)] : null; if (selectedIncident) { alreadySelected.add(selectedIncident['Task Number']); } else { logToFile("All original incidents have been selected for the current agent."); } return selectedIncident; },

  filterByCriterion = (incidents, field, value, agent, alreadySelected, triedValues = new Set(), randomServices = []) => { logToFile(`filterByCriterion - Start: field=${field}, value=${value}, agent=${agent}`); if (field === "Service" && value === "RANDOM") { const untriedServices = randomServices.filter((service) => !triedValues.has(service)); logToFile(`filterByCriterion - Untried services: ${untriedServices}`); if (untriedServices.length > 0) { value = untriedServices[Math.floor(Math.random() * untriedServices.length)]; } else { value = getRandomValue(incidents, field, alreadySelected); alreadySelected.add(value); } logToFile(`filterByCriterion - New value for RANDOM service: ${value}`); } else if (value === "RANDOM") { value = getRandomValue(incidents, field, alreadySelected); alreadySelected.add(value); } triedValues.add(value); triedServices[agent] = triedValues; const filtered = incidents.filter((incident) => { const matches = !alreadySelected.has(incident) && incident[field] === value && incident["Taken By"] === agent; return matches; }); logToFile(`filterByCriterion - Filtered incidents: ${filtered.length}`); if (!filtered.length) { const allValues = incidents.map((i) => i[field]); const untriedValues = allValues.filter((value) => !triedValues.has(value)); if (untriedValues.length === 0) { return []; } const untriedServices = randomServices.filter((service) => !triedValues.has(service)); value = field === "Service" && untriedServices.length > 0 ? untriedServices[Math.floor(Math.random() * untriedServices.length)] : getRandomValue(incidents, field, alreadySelected); if (!triedValues.has(value)) { return filterByCriterion(incidents, field, value, agent, alreadySelected, triedValues, randomServices); } } return filtered; },
  filterIncidentsByFields = (incidents, fieldCriteria, agent, alreadySelected, randomServices) => { logToFile(`filterIncidentsByFields - Start: fieldCriteria=${JSON.stringify(fieldCriteria)}, agent=${agent}, randomServices=${JSON.stringify(randomServices)}`); return Object.entries(fieldCriteria).reduce((currentIncidents, [field, value]) => { logToFile(`filterIncidentsByFields - Processing field: ${field}, value: ${value}`); return filterByCriterion(currentIncidents, field, value, agent, alreadySelected, triedServices[agent] || new Set(), randomServices); }, incidents); },
  selectIncidentsByConfiguration = (originalXlData, incidentConfigs, maxIncidents, sfAgentMapping, randomServices) => { (!Array.isArray(originalXlData) || !originalXlData.length) && (() => { throw new Error("Invalid originalXlData"); })(); const fieldToConfigKey = { Service: "service", "Contact type": "contactType", "First time fix": "ftf", }; return Object.entries(sfAgentMapping).reduce((selectedIncidents, [sfMember, agents]) => { selectedIncidents[sfMember] = agents.reduce((agentIncidents, agent) => { const alreadySelected = new Set(); agentIncidents[agent] = Array(maxIncidents).fill().reduce((incidents, _, i) => { const incidentConfig = incidentConfigs[i % incidentConfigs.length]; let fieldCriteria = {};["Service", "Contact type", "First time fix"].forEach((field) => { const configKey = fieldToConfigKey[field]; if (incidentConfig[configKey]) { fieldCriteria[field] = incidentConfig[configKey]; } }); const potentialIncidents = filterIncidentsByFields(originalXlData, fieldCriteria, agent, alreadySelected, randomServices); if (potentialIncidents.length) { const uniqueIncident = selectUniqueIncidentForAgent(potentialIncidents, alreadySelected); if (uniqueIncident) { incidents.push(uniqueIncident); alreadySelected.add(uniqueIncident['Task Number']); } else { logToFile(`Warning: No incidents available for fallback for agent ${agent}.`); } } logToFile(`selectIncidentsByConfiguration_2 - Selected ${incidents.length} incidents for agent ${agent}`); return incidents; }, []); return agentIncidents; }, {}); return selectedIncidents; }, {}); },
    ...
    Object.entries(selectedIncidents).forEach(([sfMember, agents], sfIndex) => {
      Object.entries(agents).forEach(([agent, incidents], agentIndex) => {
        incidents.forEach((incident) => {
          const row = {
            "n.": previousAgent === agent ? incidentCount++ : (incidentCount = 1),
