...
const {
  ...
  selectUniqueIncidentForAgent = (filteredIncidents = [], alreadySelected, originalIncidents = []) => { const [getUniqueIncidents, getRandomIncident] = [incidents => incidents.filter(incident => !alreadySelected.has(incident)), incidents => incidents[~~(Math.random() * incidents.length)]]; const selectedIncident = getRandomIncident((filteredIncidents.length ? filteredIncidents : originalIncidents).filter(incident => !alreadySelected.has(incident))); selectedIncident && (console.warn(selectedIncident ? "" : "All original incidents have been selected for the current agent."), alreadySelected.add(selectedIncident)); return selectedIncident; },

  filterByCriterion = (incidents, field, value, agent, alreadySelected, triedValues = new Set()) => { const helper = (value, triedValues) => { triedValues.add(value); const filtered = incidents.filter(incident => !alreadySelected.has(incident) && incident[field] === value && incident["Taken By"] === agent); const untriedValues = incidents.map(i => i[field]).filter(v => !triedValues.has(v)); return filtered.length ? filtered : (untriedValues.length ? helper(getRandomValue(incidents, field, alreadySelected), triedValues) : []); }; return helper(value === "RANDOM" ? getRandomValue(incidents, field, alreadySelected) : value, triedValues); },
